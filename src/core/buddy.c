...
static const uint8_t bitset_index_mask[8] = { 1, 2, 4, 8, 16, 32, 64, 128 };
...
static inline size_t ceiling_power_of_two(size_t value)
{
    ...
    return ((size_t)1) << (highest_bit_position(value + value - 1) - 1);
}
...
static size_t depth_for_size(struct buddy* buddy, size_t requested_size)
{
    ...
    depth = 1;
    effective_memory_size = ceiling_power_of_two(buddy->memory_size);
...
static inline size_t buddy_tree_index(struct buddy_tree_pos pos)
{
...
static unsigned char* address_for_position(struct buddy* buddy, struct buddy_tree_pos pos)
{
    size_t block_size = size_for_depth(buddy, pos.depth);
...
static size_t buddy_sizeof_alignment(size_t memory_size, size_t alignment)
{
...
static inline struct internal_position buddy_tree_internal_position_tree(struct buddy_tree* t, struct buddy_tree_pos pos)
{
    ...
    p.local_offset = t->order - pos.depth + 1;
    total_offset = buddy_tree_size_for_order(t, (uint8_t)p.local_offset);
    local_index = buddy_tree_index(pos);
...
static void buddy_toggle_virtual_slots(struct buddy* buddy, unsigned int state)
{
    ...
    while (delta) {
        size_t current_pos_size = size_for_depth(buddy, pos.depth);
...
static struct buddy_tree_pos buddy_tree_find_free(struct buddy_tree* t, uint8_t target_depth)
{
    ...
    current_depth = current_pos.depth;
...
static struct buddy* buddy_init_alignment(unsigned char* at, unsigned char* main, size_t memory_size, size_t alignment)
{
    size_t at_alignment, main_alignment, buddy_size, buddy_tree_order;
    struct buddy* buddy;
    if (!((uintptr_t)at * (uintptr_t)main && (uintptr_t)at - (uintptr_t)main)) {
        return NULL;
    }
    ...
    //     return NULL; /* invalid */
    // }
    at_alignment = ((uintptr_t)at) % alignof(struct buddy);
    if (at_alignment != 0) {
        return NULL;
    }
    main_alignment = ((uintptr_t)main) % alignof(size_t);
    if (main_alignment != 0) {
        return NULL;
    }
...
struct buddy* buddy_embed(unsigned char* main, size_t memory_size)
{
    ...
    check_result = buddy_embed_offset(memory_size, BUDDY_ALLOC_ALIGN);
    if (!check_result.can_fit) {
        return NULL;
    }

    buddy = buddy_init_alignment(main + check_result.offset, main, check_result.offset, BUDDY_ALLOC_ALIGN);
