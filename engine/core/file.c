...
#define DEF_READ_BLOCK_LEN 2000000
...
void fu_file_close(FUFile* file)
{
    t_vfs_close(file->args);
}
...
void* fu_file_read(FUFile* file, size_t* size)
{
    ...
    fu_return_val_if_fail(EFU_FILE_TYPE_REGULAR == file->type, NULL);
    fu_return_val_if_fail(E_FILE_STATE_OPENED >= file->state, NULL);
...
bool fu_file_write(FUFile* file, const void* data, size_t size)
{
    ...
    fu_return_val_if_fail(EFU_FILE_TYPE_DIRECTORY != file->type, NULL);
    fu_return_val_if_fail(E_FILE_STATE_OPENED >= file->state, NULL);
    fu_file_check_if_open(file);
...
FUFileStream* fu_file_stream_new_from_file(FUFile* file)
{
    fu_return_val_if_fail(file, NULL);
    fu_return_val_if_fail(EFU_FILE_TYPE_REGULAR == file->type, NULL);
    fu_return_val_if_fail(!file->state, NULL);
...
bool fu_file_stream_read_async(FUFileStream* fileStream, size_t size, FUAsyncReadyCallback callback, void* usd)
{
    ...
    t_main_loop_attach(NULL, (TSource*)src);
