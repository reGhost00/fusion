...
static TVertexBuffer* t_vertex_buffer_new(TApp* app)
{
    ...
    vertexBuffer->vertices = (TVertex*)mi_malloc(sizeof(TVertex) * 3);
    vertexBuffer->vertexCount = 3;
    vertexBuffer->device = app->logicalDevice;

    vertexBuffer->vertices[0] = (TVertex) { { -0.5f, -0.5f }, { 1.0f, 0.0f, 0.0f } };
    vertexBuffer->vertices[1] = (TVertex) { { 0.5f, -0.5f }, { 0.0f, 1.0f, 0.0f } };
    vertexBuffer->vertices[2] = (TVertex) { { 0.0f, 0.5f }, { 0.0f, 0.0f, 1.0f } };

    VkBufferCreateInfo bufferInfo = {
        .sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
        .size = sizeof(TVertex) * 3,
        .usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT
    };
    if (vkCreateBuffer(app->logicalDevice, &bufferInfo, NULL, &vertexBuffer->buffer)) {
        fprintf(stderr, "failed to create vertex buffer!\n");
        return NULL;
    }

    VkMemoryRequirements memRequirements;
    vkGetBufferMemoryRequirements(app->logicalDevice, vertexBuffer->buffer, &memRequirements);

    VkMemoryAllocateInfo allocInfo = {
        .sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
        .allocationSize = memRequirements.size,
        .memoryTypeIndex = find_memory_type(app->physicalDevice, memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)
    };
    if (vkAllocateMemory(app->logicalDevice, &allocInfo, NULL, &vertexBuffer->memory)) {
        fprintf(stderr, "failed to allocate vertex buffer memory!\n");
        goto out1;
    }
    if (vkBindBufferMemory(app->logicalDevice, vertexBuffer->buffer, vertexBuffer->memory, 0)) {
        fprintf(stderr, "failed to bind vertex buffer memory!\n");
        goto out1;
    }
    void* data;
    vkMapMemory(app->logicalDevice, vertexBuffer->memory, 0, memRequirements.size, 0, &data);
    memcpy(data, vertexBuffer->vertices, sizeof(TVertex) * 3);
    vkUnmapMemory(app->logicalDevice, vertexBuffer->memory);

    return vertexBuffer;
out1:
    vkDestroyBuffer(app->logicalDevice, vertexBuffer->buffer, NULL);
out2:
...
{
    printf("%s\n", __func__);
    app->framebuffers = (VkFramebuffer*)mi_calloc(app->swapchainImageCount, sizeof(VkFramebuffer));
...
static void t_vertex_buffer_init_description(TVertexBuffer* buffer, VkPipelineVertexInputStateCreateInfo* description)
{
...
static void t_app_init_pipeline(TApp* app)
{
    ...
    t_vertex_buffer_init_description(app->vertexBuffer, &vertexInputState);
...
private:
    ...
    void initVulkan()
    {
        ...
        vertexBuffer = app->vertexBuffer->buffer;
        t_app_init_pipeline(app);
        pipelineLayout = app->pipelineLayout;
        graphicsPipeline = app->pipeline;
    ...
    {
        auto vertShaderCode = readFile("D:/dev/C/fusion/test/gl1/vert.spv");
        auto fragShaderCode = readFile("D:/dev/C/fusion/test/gl1/frag.spv");
        ...
        t_vertex_buffer_init_description(app->vertexBuffer, &vertexInputInfo);
    ...
    {
        TVertexBuffer* vb = (TVertexBuffer*)mi_malloc(sizeof(TVertexBuffer));
        vb->vertices = (TVertex*)mi_malloc(sizeof(TVertex) * 3);
        vb->vertexCount = 3;
        vb->device = app->logicalDevice;

        vb->vertices[0] = (TVertex) { { -0.5f, -0.5f }, { 1.0f, 0.0f, 0.0f } };
        vb->vertices[1] = (TVertex) { { 0.5f, -0.5f }, { 0.0f, 1.0f, 0.0f } };
        vb->vertices[2] = (TVertex) { { 0.0f, 0.5f }, { 0.0f, 0.0f, 1.0f } };

        VkBufferCreateInfo bufferInfo {};
        bufferInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
        // bufferInfo.size = sizeof(vertices[0]) * vertices.size();
        bufferInfo.size = sizeof(TVertex) * vb->vertexCount;
        ...
        allocInfo.memoryTypeIndex = find_memory_type(physicalDevice, memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
        ...
        memcpy(data, vb->vertices, (size_t)bufferInfo.size);
