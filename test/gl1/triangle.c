...
static TVertexBuffer* t_vertex_buffer_new(TApp* app)
{
    ...
    if (!t_buffer_init(&bufferArgs, (TBufferResult*)&vertexBuffer->buffer)) {
        fprintf(stderr, "failed to create vertex buffer!\n");
        goto out;
    }

    t_buffer_copy(stagingBuffer.buffer, vertexBuffer->buffer, sizeof(TVertex) * 3, app);
    ...
    // if (!t_buffer_new(&bufferArgs, (TBufferResult*)&vertexBuffer->buffer)) {
    //     fprintf(stderr, "failed to create vertex buffer!\n");
    //     goto out;
    // }

    // void* data;
    // vkMapMemory(app->logicalDevice, vertexBuffer->memory, 0, bufferArgs.size, 0, &data);
